...
IEnumerable<Eruption> findTheFirst = eruptions.Where(c => c.Location =="Chile");
PrintEach(findTheFirst);
IEnumerable<Eruption> findHawain = eruptions.Where(c => c.Location =="Hawaiian Is");
PrintEach(findHawain);
...
IEnumerable<Eruption> orderedVolcano = eruptions.OrderBy(prod => prod.Volcano);
PrintEach(orderedVolcano);
IEnumerable<Eruption> orderedbyVolcano = eruptions.OrderBy(prod => prod.Volcano).OrderBy(h=>h.Year<1000);
PrintEach(orderedbyVolcano);
IEnumerable<string> AllTitles = orderedbyVolcano.Select(y=>y.Volcano);
PrintEach(AllTitles);
